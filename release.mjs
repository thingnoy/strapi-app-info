...
(async () => {
  ...
  const gitCommitterName = GIT_COMMITTER_NAME || 'release-bot'
  const gitCommitterEmail = GIT_COMMITTER_EMAIL || 'release-bot@robot.com'
  const originUrl = (await $`git config --get remote.origin.url`).toString().trim()
  const branch = GIT_BRANCH || (await $`git branch --show-current`).toString().trim() || 'main'
